        ...
        <Col span={8}>
          <div className={style.left}>
            ...
            >
              {options.map((option) => {
                return (
                  <Popconfirm title="添加权限至"
                              onConfirm={this.handleSelect.bind(this, 1, option)}
                              onCancel={this.handleSelect.bind(this, 2, option)}
                              okText="读写" cancelText="只读">
                    <option value={option.realName} key={option.realName} disabled={disabled}>{option.realName}</option>
                  </Popconfirm>
                )
              })}
            </select>
